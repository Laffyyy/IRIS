...
class ChangePasswordService {
    ...
    // For the updateFirstTimeUser method
async updateFirstTimeUser(userID, newPassword, securityQuestions) {
    try {
      ...
      // Determine which table to use: first check tbl_login
      let table = 'tbl_login';
      let currentUser;
      
      const [loginUserCheck] = await db.query(
        'SELECT dPassword1_hash, dPassword2_hash, dPassword3_hash FROM tbl_login WHERE dUser_ID = ?', 
        [userID]
      );
          
      if (loginUserCheck.length === 0) {
        // If not in tbl_login, check tbl_admin
        const [adminUserCheck] = await db.query(
          'SELECT dPassword1_hash, dPassword2_hash, dPassword3_hash FROM tbl_admin WHERE dUser_ID = ?', 
          [userID]
        );
        ...
        table = 'tbl_admin';
        currentUser = adminUserCheck;
      } else {
        currentUser = loginUserCheck;
      }
      ...
      await db.query(`SET time_zone = '+08:00'`);
      // Update logs (make sure this works for both tables)
      await db.query(
...
async changeSecurityQuestions(userID, securityQuestions) {
    try {
        ...
        const [loginUserCheck] = await db.query(
            'SELECT 1 FROM tbl_login WHERE dUser_ID = ?', 
            [userID]
        );
        ...
        if (loginUserCheck.length === 0) {
            // If not in tbl_login, check tbl_admin
            const [adminUserCheck] = await db.query(
                'SELECT 1 FROM tbl_admin WHERE dUser_ID = ?', 
                [userID]
            );
    ...
    async verifySecurityAnswer(userId, questionId, answer) {
        try {
            ...
            if (loginResult.length > 0 && loginResult[0].hashedAnswer) {
                return await bcrypt.compare(answer, loginResult[0].hashedAnswer);
            }
            
            // If not found or no match in tbl_login, try tbl_admin
            ...
            if (adminResult.length === 0 || !adminResult[0].hashedAnswer) {
                return false;
            }
    
            // Compare provided answer with stored hash
            return await bcrypt.compare(answer, adminResult[0].hashedAnswer);
        } catch (error) {
